...
setwd("/ibex/scratch/qadirga/Project_4/Data_Application/MyModel")
#setwd("/Users/qadirga/Documents/Project 4-Spacetime modeling/Revisit/Simulation Study/Revisit2/Revisit 3/Data Application/MyModel")
...
tr.days<-355
alpha.poly.order<-nu.poly.order<-5

xmatrix.alpha<-matrix(NA, nrow = tr.days,ncol = alpha.poly.order-1)
xmatrix.nu<-matrix(NA, nrow = tr.days,ncol = nu.poly.order-1)


for(i in 1:(alpha.poly.order-1))
{
  xmatrix.alpha[,i]<-tseq[1:tr.days]^(i)
}

for(i in 1:(nu.poly.order-1))
{
  xmatrix.nu[,i]<-tseq[1:tr.days]^(i)
}
...
alpha.ldmat<-cbind(alpha.logy,xmatrix.alpha)
nu.ldmat<-cbind(nu.logy,xmatrix.alpha)

alpha.ldmat<-as.data.frame(alpha.ldmat)
nu.ldmat<-as.data.frame(nu.ldmat)


alpha.lmestim<-lm(alpha.logy~.,data = alpha.ldmat)
nu.lmestim<-lm(nu.logy~.,data = nu.ldmat)
par(mfrow=c(1,2))
plot(tseq[1:tr.days],ls.a,ylab="alpha_s(t)",xlab="t (days)",main="Spatial scale",type="l")
lines(tseq[1:tr.days],alpha.func(t=tseq[1:tr.days],params = alpha.lmestim$coefficients),col="green")  
...
lines(tseq[1:tr.days],nu.func(t=tseq[1:tr.days],params = nu.lmestim$coefficients),col="green")  

log(ls.nu[1])
log(ls.a[1])
log(1.5)
ini.v<-c(alpha.lmestim$coefficients,nu.lmestim$coefficients,sd(res.mat))
  
names(ini.v)<-NULL
train.dist<-rdist(cbind(my.res.data[,1],my.res.data[,2]))
nu.func(t=tseq,params=nu.lmestim$coefficients)
alpha.func(t=tseq,params = alpha.lmestim$coefficients)
...
mle.comp.all<-function(tr.dist=train.dist,z=c(res.mat),p,tl=tdays,sl=tr.l,tseq.arg=tseq)
{
  ...
  a.s<-alpha.func(t=tseq.arg[1:tl],params = c(a.0,a.1,a.2,a.3,a.4))
  nu.s<-nu.func(t=tseq.arg[1:tl],params=c(nu.0,nu.1,nu.2,nu.3,nu.4))
  ...
  else
  {
    ...
    daywise.nlogl<-foreach(i=1:tl,.packages = c("mvtnorm"))%dopar%
    {
      ...
      temp<--dmvnorm(x=tspec.d,mean=(rep(0,times=length(tspec.d))),
                     sigma =my.matern(h=tr.dist,a=a.s[i],nu=nu.s[i],sigma = sigma) ,log = T,checkSymmetry = F)
      #rm(myC,tspec.d)
...
#############################################
#save.image("Oregan_mlep012.RData")
#load("Oregan_mlep012.RData")

######## Now reporting preliminary estimates ######

#ml1.a<-ml1.nu<-ml1.sigma<-numeric()

#ml1.a<-alpha.func(t=tseq[1:tdays],params = myspace.estims$par[1:3])
#ml1.nu<-nu.func(t=tseq[1:tdays],params = myspace.estims$par[4:6])
#ml1.sigma<-myspace.estims$par[7]
...
#system.time(mle.temporal.all(p=c(10,0.9,0.5,0.5)))
...
set.seed(1)
ls.ini<-runif(4,min = 0.1,max = 0.9)
...
#adhoc_temp<-zeta(t=tseq[1:tdays],nus=ls.nu,alphas = ls.a,a=tempporal.est$par[1],alpha =tempporal.est$par[2],beta = tempporal.est$par[3],
                # beta2 = tempporal.est$par[4],c.p = mean(ls.a))
...
names(alpha.lmestim$coefficients)<-NULL
names(nu.lmestim$coefficients)<-NULL

#exp(0.1)
my.p1.ini<-c(alpha.lmestim$coefficients,nu.lmestim$coefficients,(mygm_ini[3]))
#alpha.func(t=tseq[1:tdays],params = c(log(mygm_ini[1]),0,0))
#nu.func(t=tseq[1:tdays],params = c(log(mygm_ini[2]),0,0))
my.p2.ini<-c(mygm_ini[4],mygm_ini[5],mygm_ini[6],mygm_ini[7])
                      ...
                      myloc.batch=loc.batch.ind,z=c(res.mat),tl=tdays,sl=tr.l,tseq.arg=tseq,tr.dist=train.dist)
    {
      ...
      #distmat<-rdist(locs)
      a.0<-p[1]*as.numeric(p[1]>-2.30&p[1]<0.40)-2.30*as.numeric(p[1]<=-2.30)+0.40*as.numeric(p[1]>=0.40)
      a.1<-p[2]
      a.2<-p[3]
      a.3<-p[4]
      a.4<-p[5]
      nu.0<-p[6]*as.numeric(p[6]>-1&p[6]<0)-1*as.numeric(p[6]<=-1)+0*as.numeric(p[6]>=0)
      nu.1<-p[7]
      nu.2<-p[8]
      nu.3<-p[9]
      nu.4<-p[10]
      ...
      if(sum(c(sigma<=0,mya_bern1<=0,mya_bern1<=0,myalpha_bern1<=0,myalpha_bern1>1,mybeta_bern1<0,mybeta_bern1>1,mybeta2_bern1<0))!=0)
      {
        return(list(mlv=Inf))
      }
        ...
        cat("done with nlogl2 ",p,"value=",(nlogl1+myloglv)/2,"\n")
             return(list(mlv=(nlogl1+myloglv)/2))
          }
    
    } 
    
    
    system.time(mle_rcl(p=c(my.p1.ini,my.p2.ini)))
    #-201319.3
    
    mle.rcl_only_mlv<-function(par)
    {
      return(mle_rcl(p=par)
      )
    }
    ...
    #trialp<-c(-1.890971, 19.586, -100.6774, 153.5456, -71.83763, 15.06487, 0.9705362, -8.389941, 15.69335, -8.29979, 0.682874, 20.75965, 0.2311537, 0.4931019, 8.406898e-07)
    #system.time(mle.rcl_only_mlv(myrcl.ini.v))
    #mle.rcl_only_mlv(trialp)
  ...
  rcl.est<-optim_rcl.loglik(par=myrcl.ini.v)
    ...
    save.image("Myrcl_ests_gm_ini6_2b.RData")
