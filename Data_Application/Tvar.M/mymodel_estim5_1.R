...
tr.days<-355
alpha.poly.order<-nu.poly.order<-3
...
mle.comp.all<-function(tr.dist=train.dist,z=c(res.mat),p,tl=tdays,sl=tr.l,tseq.arg=tseq)
{
  ...
  a.s<-alpha.func(t=tseq.arg[1:tl],params = c(a.0,a.1,a.2))
  nu.s<-nu.func(t=tseq.arg[1:tl],params=c(nu.0,nu.1,nu.2))
...
my.p2.ini<-c(mygm_ini[4],mygm_ini[5],mygm_ini[6],mygm_ini[7])
    ...
    save.image("Myrcl_ests_gm_ini5_1.RData")
