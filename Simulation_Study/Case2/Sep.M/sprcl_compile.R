...
#setwd("/Users/qadirga/Documents/Project 4-Spacetime modeling/Revisit/Simulation Study/Revisit2/Revisit 3/Case2_2/FInal piece of codes/SeparableModel")
library(MASS)
library(mvtnorm)
library(fields)
library(doParallel)
...
setwd("/ibex/scratch/qadirga/Project_4/Simulation_study_rev3/Case2_2/SeparableModel")
...
sprcl<-do.call(c,list(sprcl1,sprcl2,sprcl3))
#getwd()
...
for(i in 1:nsim)
{
  ...
  alphas.rcl[i]<-sprcl[[i]]$estimates$par[1]
  nus.rcl[i]<-sprcl[[i]]$estimates$par[2]
  sigma.rcl[i]<-sprcl[[i]]$estimates$par[3]
  
  bern_alpha.rcl[i]<-sprcl[[i]]$estimates$par[4]
  bern_beta1.rcl[i]<-0
  bern_beta2.rcl[i]<-sprcl[[i]]$estimates$par[5]
  
  myest.a.rcl[i,]<-alphas.rcl[i]
  myest.nu.rcl[i,]<-nus.rcl[i]
  count_f[i]<-sprcl[[i]]$estimates$counts[1]
}
...
mypredres<-foreach(simkey=1:nsim,.packages = c("sp","gstat","geoR","fields","MASS","mvtnorm","scoringRules"))%dopar%{
  ...
  runi.zeta<-zeta(t=tseq,nus = myest.nu.rcl[simkey,],alphas = myest.a.rcl[simkey,],a=mya_bern,alpha=bern_alpha.rcl[simkey],beta=bern_beta1.rcl[simkey],c.p = mean(myest.a.rcl[simkey,]),beta2 = bern_beta2.rcl[simkey])
  #par(mfrow=c(1,2))
...
save.image("separablemodel_rcl_pred.RData")

load("separablemodel_rcl_pred.RData")



seppred_crps<-seppred_logs<-sepforc_crps<-sepforc_logs<-numeric()

for(i in 1:100)
{
  seppred_crps[i]<-mean(crps_norm(y=c(mypredres[[i]]$testval_pred),mean = c(mypredres[[i]]$pred.val_pred),sd=sqrt(c(mypredres[[i]]$pred.var_pred))))
  seppred_logs[i]<-mean(logs_norm(y=c(mypredres[[i]]$testval_pred),mean = c(mypredres[[i]]$pred.val_pred),sd=sqrt(c(mypredres[[i]]$pred.var_pred))))
  
  sepforc_crps[i]<-mean(crps_norm(y=c(mypredres[[i]]$testval_forc),mean = c(mypredres[[i]]$pred.val_forc),sd=sqrt(c(mypredres[[i]]$pred.var_forc))))
  sepforc_logs[i]<-mean(logs_norm(y=c(mypredres[[i]]$testval_forc),mean = c(mypredres[[i]]$pred.val_forc),sd=sqrt(c(mypredres[[i]]$pred.var_forc))))
...
par(mfrow=c(1,1))
boxplot(mypred_crps,gmpred_crps,seppred_crps,myforc_crps,gmforc_crps,sepforc_crps,col = c("blue","blue","blue","grey","grey","grey"),names=c("Tvar","Gneit","Sep","Tvar","Gneit","Sep"),main="mCRPS")
grid()
abline(v=3.5)
legend("bottomright",pch=c(19,19),col=c("blue","grey"),c("Prediction","Forcasting"))
boxplot(mypred_logs,gmpred_logs,seppred_logs,myforc_logs,gmforc_logs,sepforc_logs,col = c("blue","blue","blue","grey","grey","grey"),names=c("Tvar","Gneit","Sep","Tvar","Gneit","Sep"),main="mLogS")
grid()
abline(v=3.5)
legend("bottomright",pch=c(19,19),col=c("blue","grey"),c("Prediction","Forcasting"))
