...
#alphas<-rep(20,times=time_points)
alphas<-20-10*exp(10*(tseq-0.5))/(1+exp(10*(tseq-0.5)))

#### fitting loglinear model ####
#temp.x<-data.frame(y=log(alphas),x2=tseq,x3=tseq^2,x4=tseq^3)
#temp.lm<-lm(y~1+x2+x3,data = temp.x)
#temp.lm
#emp.alpha<-exp(3.0748-0.6624*tseq-0.2101*tseq*tseq)
#plot(tseq,20-10*exp(10*(tseq-0.5))/(1+exp(10*(tseq-0.5))),ylim=c(5,25))
#points(tseq,emp.alpha,col="red",pch=19)

#alphas<-25-10*tseq
#nus<-mynus.f(t_i=tseq,nus.init=nus.init,nus.max=nus.max)
#nus<-rep(1,times=time_points)
nus<-0.5+1*exp(10*(tseq-0.5))/(1+exp(10*(tseq-0.5)))
plot(tseq,nus,ylim = c(0.4,1.7))
#temp.x<-data.frame(y=log(nus),x2=tseq,x3=tseq^2,x4=tseq^3)
#temp.lm<-lm(y~1+x2+x3+x4,data = temp.x)
#temp.lm
#emp.alpha<-exp(-0.6761 -0.5508*tseq+5.7320*tseq*tseq-4.1674*(tseq^3))
#plot(tseq,20-10*exp(10*(tseq-0.5))/(1+exp(10*(tseq-0.5))),ylim=c(5,25))
#points(tseq,emp.alpha,col="red",pch=19)
