...
for(i in 1:nsim)
{
  ...
  a.3[i]<-mymargins[[i]]$estimate$par[4]
  nu.0[i]<-mymargins[[i]]$estimate$par[5]
  nu.1[i]<-mymargins[[i]]$estimate$par[6]
  nu.2[i]<-mymargins[[i]]$estimate$par[7]
  nu.3[i]<-mymargins[[i]]$estimate$par[8]
...
myrcl_estims<-foreach(simkey=37:72,.packages = c("sp","gstat","geoR","fields","MASS","mvtnorm"))%dopar%
  {
    
    start.time<-proc.time()
    ########## Using composite loglikelihood ###########
    
    my.p1.ini<-c(a.0[simkey],a.1[simkey],a.2[simkey],a.3[simkey],nu.0[simkey]
                 ,nu.1[simkey]
                 ,nu.2[simkey],nu.3[simkey]
                 ,myest.sigma[simkey])
                      ...
                      myspat1,mynbatch=nbatch,myloc.batch=loc.batch.ind[,,simkey])
    {
      ...
      a.est<-a<-exp(a.0+a.1*tseq[1:tl]+a.2*((tseq[1:tl])^2)+a.3*((tseq[1:tl])^3))
      nu.est<-nu<-exp(nu.0+nu.1*tseq[1:tl]+nu.2*(tseq[1:tl]^2)+nu.3*(tseq[1:tl]^3))
