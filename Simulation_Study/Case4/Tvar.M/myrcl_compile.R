...
setwd("/ibex/scratch/qadirga/Project_4/Simulation_study_rev3/Case4_2/MyModel")
#setwd("/Users/qadirga/Documents/Project 4-Spacetime modeling/Revisit/Simulation Study/Revisit2/Revisit 3/Case4_2/FInal piece of codes/MyModel")
library(MASS)
library(mvtnorm)
library(fields)
library(doParallel)
...
a.0.rcl<-a.1.rcl<-a.2.rcl<-a.3.rcl<-nu.0.rcl<-nu.1.rcl<-nu.2.rcl<-nu.3.rcl<-bern_alpha.rcl<-bern_beta1.rcl<-bern_beta2.rcl<-sigma.rcl<-numeric(length = nsim)
...
for(i in 1:nsim)
{
  ...
  a.3.rcl[i]<-myrcl_f[[i]]$estimates$par[4]
  nu.0.rcl[i]<-myrcl_f[[i]]$estimates$par[5]
  nu.1.rcl[i]<-myrcl_f[[i]]$estimates$par[6]
  nu.2.rcl[i]<-myrcl_f[[i]]$estimates$par[7]
  nu.3.rcl[i]<-myrcl_f[[i]]$estimates$par[8]
  sigma.rcl[i]<-myrcl_f[[i]]$estimates$par[9]

  bern_alpha.rcl[i]<-myrcl_f[[i]]$estimates$par[10]
  bern_beta1.rcl[i]<-myrcl_f[[i]]$estimates$par[11]
  bern_beta2.rcl[i]<-myrcl_f[[i]]$estimates$par[12]
  
  myest.a.rcl[i,]<-exp(a.0.rcl[i]+a.1.rcl[i]*tseq[1:tl]+a.2.rcl[i]*((tseq[1:tl])^2)+a.3.rcl[i]*((tseq[1:tl])^3))
  myest.nu.rcl[i,]<-exp(nu.0.rcl[i]+nu.1.rcl[i]*tseq[1:tl]+nu.2.rcl[i]*(tseq[1:tl]^2)+nu.3.rcl[i]*(tseq[1:tl]^3))
...
c3true_alpha<-alphas
c3myalpha.mean<-myest.a.rcl.mean
c3myalpha.sd<-myest.a.rcl.sd
c3true_nus<-nus
c3mynu.mean<-myest.nu.rcl.mean
c3mynu.sd<-myest.nu.rcl.sd

#rm(list = ls()[!ls()%in%c("tseq","c3true_alpha","c3myalpha.mean","c3myalpha.sd","c3true_nus","c3mynu.mean","c3mynu.sd")])
#save.image("c3myalphnu.RData")
