...
myalpha.d<-data.frame(y=log(est.a),x1=tseq[1:(tl-2)],x2=tseq[1:(tl-2)]^2,x3=tseq[1:(tl-2)]^3)

mynus.d<-data.frame(y=log(est.nu),x1=tseq[1:(tl-2)],x2=tseq[1:(tl-2)]^2,x3=tseq[1:(tl-2)]^3)

alpha.lm<-lm(y~1+x1+x2+x3,data = myalpha.d)
nus.lm<-lm(y~1+x1+x2+x3,data = mynus.d)
...
my.comp.ini2<-c(as.numeric(alpha.lm$coefficients[1]),as.numeric(alpha.lm$coefficients[2]),as.numeric(alpha.lm$coefficients[3]),as.numeric(alpha.lm$coefficients[4]),as.numeric(nus.lm$coefficients[1])
,as.numeric(nus.lm$coefficients[2])
,as.numeric(nus.lm$coefficients[3])
,as.numeric(nus.lm$coefficients[4])
...
mle.comp.all<-function(locs,z,p,tl=(tl-2),sl=train_l,mat.index=mat.index.tr,uniq.dist=uniq.dist.tr,tseq=tseq)
{
  ...
  #nu<-p[(tl+1):(2*tl)]
  sigma<-p[9]
  a<-exp(a.0+a.1*tseq[1:tl]+a.2*tseq[1:tl]^2+a.3*tseq[1:tl]^3)
  nu<-exp(nu.0+nu.1*tseq[1:tl]+nu.2*tseq[1:tl]^2+nu.3*tseq[1:tl]^3)
...
for(i in 1:nsim)
{
  ...
  a.3<-mymargins[[i]]$estimate$par[4]
  nu.0<-mymargins[[i]]$estimate$par[5]
  nu.1<-mymargins[[i]]$estimate$par[6]
  nu.2<-mymargins[[i]]$estimate$par[7]
  nu.3<-mymargins[[i]]$estimate$par[8]
  
  myest.sigma[i]<-mymargins[[i]]$estimate$par[9]
  myest.a[i,]<-exp(a.0+a.1*tseq[1:(tl-2)]+a.2*tseq[1:(tl-2)]^2+a.3*tseq[1:(tl-2)]^3)
  myest.nu[i,]<-exp(nu.0+nu.1*tseq[1:(tl-2)]+nu.2*tseq[1:(tl-2)]^2+nu.3*tseq[1:(tl-2)]^3)
}
#############################################
