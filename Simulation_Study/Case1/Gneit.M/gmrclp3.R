...
setwd("/ibex/scratch/qadirga/Project_4/Simulation_study_rev3/Case1_2/GneitingModel")

#save.image("gneitingtemporal.RData")
load("gneitingtemporal.RData")
...
######### Random composite log-likelihood ########

gmrcl<-foreach(simkey=73:100,.packages = c("sp","gstat","geoR","fields","MASS","mvtnorm"))%dopar%
  {
    
    start.time<-proc.time()
    
    
    gm.p1.ini<-c(myest.a[simkey,1],myest.nu[simkey,1],myest.sigma[simkey])
    
    gm.p2.ini<-c(bern.est.alpha[simkey],bern.est.beta1[simkey],bern.est.beta2[simkey])
    #### Computing mat.index for train.locs ####
    gm.fin.ini<-c(gm.p1.ini,gm.p2.ini)
    ...
    #(locs,p,sl=train_l,mat.index=mat.index.tr,uniq.dist=uniq.dist.tr,tseq=tseq,
    #  myspat1,mynbatch=nbatch,myloc.batch=loc.batch.ind[,,simkey])
    
    mle_gm_rcl<-function(locs,p,sl=train_l,mat.index=mat.index.tr,uniq.dist=uniq.dist.tr,tseq=tseq,
                         myspat1,mynbatch=nbatch,myloc.batch=loc.batch.ind[,,simkey])
    {
      #distmat<-rdist(locs)
      a<-p[1]
    ...
    mle.rcl_gm_only_mlv<-function(par)
    {
      return(mle_gm_rcl(locs=train.locs[,,simkey],p=par,sl=train_l,mat.index=mat.index.tr,
                     myspat1=train.myspat[,,simkey],uniq.dist=uniq.dist.tr
                     ,tseq=tseq,mynbatch=nbatch,myloc.batch=loc.batch.ind[,,simkey])
      )
    }
    
    #system.time(mle.rcl_gm_only_mlv(gm.fin.ini))
    
    optim_rcl.gm.loglik <- function(par){
      optim(par=par,
            fn = mle.rcl_gm_only_mlv,
            hessian=FALSE,
    ...
    rcl.est<-optim_rcl.gm.loglik(par=gm.fin.ini)
